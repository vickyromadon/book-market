...
@section('content')
                    ...
                    <div class="modal-body">
                        <div class="form-horizontal">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" id="name" name="name" class="form-control" placeholder="Masukkan Nama Kecamatan">
                                    <span class="help-block"></span>
                                </div>
                            </div>
                        </div>
    ...
    <div class="modal fade" tabindex="-1" role="dialog" id="modalDelete">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ route('admin.sub-district.destroy', ['id' => '#']) }}" method="post" id="formDelete">
                	{{ method_field('DELETE') }}
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                    ...
                    <div class="modal-body">
                        <p id="del-success">Anda yakin ingin menghapus Kecamatan ?</p>
                    </div>
                    <div class="modal-footer">
	...
	<script>
        jQuery(document).ready(function($){
            var table = $('#data_table').DataTable({
                ...
                "lengthChange": true,
                "ajax": {
                    "url": "{{ route('admin.sub-district.index') }}",
                    "type": "POST",
                    "data" : {}
                },
                "language": {
            ...
            $('#btnAdd').click(function () {
                $('#formAdd')[0].reset();
                $('#formAdd .modal-title').text("Tambah Kecamatan");
                ...
                url = '{{ route("admin.sub-district.store") }}';
            ...
            $('#data_table').on('click', '.edit-btn', function(e){
                $('#formAdd div.form-group').removeClass('has-error');
                $('#formAdd .help-block').empty();
                $('#formAdd .modal-title').text("Ubah Kecamatan");
                $('#formAdd')[0].reset();
                ...
                url = '{{ route("admin.sub-district.index") }}' + '/' + aData.id;

                $('#id').val(aData.id);
                $('#name').val(aData.name);
                $('#district_id').val(aData.district_id);
